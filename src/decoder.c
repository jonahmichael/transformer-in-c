...
DecoderBlock* decoder_create(int d_model, int d_ff, int num_heads) {
    DecoderBlock* decoder = calloc(1, sizeof(DecoderBlock));
...
void decoder_free(DecoderBlock* dec) {
    ...
    tensor_free(dec->W1); tensor_free(dec->W2);
    free(dec->b1); free(dec->b2);
    free(dec);
}
...
Tensor* decoder_forward(DecoderBlock* dec, Tensor* target, Tensor* enc_output) {
    ...
    tensor_free(mha2_out); tensor_free(add1);
    ...
    tensor_free(ff2); tensor_free(add2);
