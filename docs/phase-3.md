...
(i will explain to you in detail in my blog..)
...
hint: The return type shouldn't be Tensor* — we're creating a MultiHeadAttention, not a Tensor
...
- Create output tensor of shape (X->rows, d_k)
...
If each head output is (seq_len, d_k) and we have h heads, the output is (seq_len, h*d_k) = (seq_len, d_model).

Try writing the signature:
...
- Create output of shape (heads[0]->rows, h * d_k)
...
Tensor* mha_forward(MultiHeadAttention* mha, Tensor* Q, Tensor* K, Tensor* V) {
    ...
    // Step 3: Concat all head outputs
...
for (int i = 0; i < mha->h; i++) {
    // slice Q_proj, K_proj, V_proj for this head
    // run attention
...
Q_proj (seq_len, d_model)
        ↓ split into h heads
Q_0  Q_1  Q_2 ... Q_h    each (seq_len, d_k)
 ↓    ↓    ↓        ↓
att  att  att  ... att    attention per head
 ↓    ↓    ↓        ↓
